...
//     super.onInit();
//     fetchMyBookings();
//     fetchIndianCities(); // Now using real API
//     fetchFreeBookings();
//   }

// //fetch free booking

// // NEW: Fetch Free Bookings
//   Future<void> fetchFreeBookings({bool showLoading = false}) async {
...
//           .get(
//             Uri.parse("$appurl/free-booking/my?user_id=$userId"),
//           )
...
//             final String startTimeStr = item["start_time"]?.toString() ?? "";
//             final String endTimeStr = item["end_time"]?.toString() ?? "";
//             final String addedOn = item["added_on"]?.toString() ?? "";
...
//             try {
//               // Parse start and end times (they include date + time)
//               if (startTimeStr.isNotEmpty) {
//                 startDateTime =
//                     DateTime.parse(startTimeStr.replaceAll(" ", "T"));
//               }
//               if (endTimeStr.isNotEmpty) {
//                 endDateTime = DateTime.parse(endTimeStr.replaceAll(" ", "T"));
//               }

//               // Use end_time for past check (preferred)
//               if (endDateTime != null) {
//                 isPast = endDateTime.isBefore(now);
//                 displayDate =
//                     endDateTime.toString().split(" ")[0]; // YYYY-MM-DD
//               } else if (startDateTime != null) {
//                 // Fallback: if no end_time, use start
//                 isPast = startDateTime.isBefore(now);
//                 displayDate = startDateTime.toString().split(" ")[0];
//               } else if (addedOn.isNotEmpty && addedOn.contains(" ")) {
//                 // Last fallback: added_on date
//                 displayDate = addedOn.split(" ")[0];
//                 final fallbackDate = DateTime.parse(displayDate);
//                 isPast = fallbackDate.isBefore(now);
//               }
//             } catch (e) {
//               // Parsing failed → keep defaults
//               debugPrint("Date parse error for free booking ${item["id"]}: $e");
//             }

//             final bool isTwoWay =
//                 endTimeStr.trim().isNotEmpty && endTimeStr != startTimeStr;

//             String avatarUrl =
//                 carImages[item["car_type"]] ?? "$imageurlstatic/hatchback.png";

//             // TODO: Replace with real data if you have API to fetch driver info
...
//               'driver': driverName,
//               'mobile': mobile,
//               'from': item["location"] ?? "Free Vehicle Location",
//               'to': "", // No destination for free bookings
...
//           freeBookings.assignAll(parsed);
//         }
//       } else {
//         debugPrint("Free bookings failed - status: ${response.statusCode}");
//       }
//     } catch (e) {
//       debugPrint("Free bookings error: $e");
//     } finally {
...
//       fetchFreeBookings(); // called in both success and failure
//     }
//   }
